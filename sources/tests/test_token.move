module movegpt::test_token {
    ...
    #[test(deployer = @0xcafe,sender = @0x123, recipient = @0xdead, operator = @0xcafe)]
    fun test_e2e(deployer: &signer, sender: &signer, recipient: &signer, operator: &signer) {
        ...
        movegpt_token::freeze_token(operator, recipient_addr);
        assert!(coin::is_coin_store_frozen<MovegptCoin>(recipient_addr), 0);
        movegpt_token::unfreeze_token(operator, recipient_addr);
        assert!(!coin::is_coin_store_frozen<MovegptCoin>(recipient_addr), 0);
        movegpt_token::set_operator(deployer, sender_addr);
        movegpt_token::freeze_token(sender, recipient_addr);
        assert!(coin::is_coin_store_frozen<MovegptCoin>(recipient_addr), 0);
    }

    #[test(deployer=@0xcafe, operator=@0xcafe1, fake_operator=@0xcafe2)]
    #[expected_failure(abort_code = 1,location=movegpt_token)]
    public entry fun test_token_freeze_fail_by_auth(deployer: &signer, operator: &signer, fake_operator: &signer) {
        test_helper::setup(deployer,signer::address_of(operator));
        movegpt_token::freeze_token(fake_operator, signer::address_of(operator));
    }
}